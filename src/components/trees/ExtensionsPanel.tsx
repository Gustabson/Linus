...
import { Plus, X, Link, Smartphone, Image, Video, Wrench, FileText, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-900">Extensiones</h2>
        {isOwner && (
          <button
      ...
      {extensions.length === 0 ? (
        <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-8 text-center text-gray-400 text-sm">
          No hay extensiones todavía.
          {isOwner && " Agregá links, apps o recursos que complementen este currículo."}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {extensions.map((ext) => (
            <div
              key={ext.id}
              className="bg-white rounded-xl border border-gray-200 p-4 flex items-start gap-3 group"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-medium text-gray-900 text-sm">{ext.title}</p>
                    {ext.description && (
                      <p className="text-gray-500 text-xs mt-0.5">{ext.description}</p>
                    )}
                    <p className="text-xs text-gray-400 mt-1">
                      {TYPE_LABELS[ext.type]}
                    </p>
                  </div>
