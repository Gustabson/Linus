...
import { CorreosComposer } from "./CorreosComposer";
...
export function CorreosSidebar({ unreadCount }: Props) {
  ...
  function isActive(href: string) {
    return href === "/correos" ? pathname === "/correos" : pathname.startsWith(href);
  }
  ...
  return (
    <>
      <aside className="w-56 shrink-0 border-r border-gray-200 flex flex-col bg-white">
        ...
        <div className="p-4">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-green-700 hover:bg-green-800 text-white font-semibold text-sm px-4 py-2.5 rounded-2xl shadow-sm transition-colors"
          >
      ...
      {/* Composer modal */}
      {showComposer && (
        <CorreosComposer onClose={() => setShowComposer(false)} />
      )}
    </>
  );
}
